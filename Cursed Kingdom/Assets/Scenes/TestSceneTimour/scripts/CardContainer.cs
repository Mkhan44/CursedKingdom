...
public class CardContainer : MonoBehaviour
{
    ...
    [SerializeField]
    private bool disableCardWrapper;
    ...
    private bool lastDisableCardWrapper;
    ...
    {
        var wasFirstSetup = isFirstSetup;
        SetUpCards();
        SetCardsAnchor();
        if (wasFirstSetup)
        {
    ...
    void SetUpCards()
    {
        // Snapshot existing wrappers BEFORE clearing, so we can tell which cards are truly new
        var existingWrappers = new HashSet<CardWrapper>(cards);
        cards.Clear();
        foreach (Transform card in transform)
        {
            var wrapper = card.GetComponent<CardWrapper>();
            bool isNewCard = wrapper == null || !existingWrappers.Contains(wrapper);
            if (wrapper == null)
            {
            ...
            wrapper.preventCardInteraction = disableCardWrapper;
            wrapper.container = this;

            // Only play the entry animation for newly added cards, and never on first setup
            if (isNewCard && !isFirstSetup)
            {
                // Hide off-screen immediately so it's never visible at its default position
                wrapper.MoveOffScreen();
                // Defer the tween by one frame so SetCardsPosition() can set targetPosition first
                StartCoroutine(PlayEntryNextFrame(wrapper));
            }
        }
        isFirstSetup = false;
    }

    private void SnapCardsToPosition()
    {
    ...
    private IEnumerator PlayEntryNextFrame(CardWrapper wrapper)
    {
        yield return null; // wait one frame so SetCardsPosition() runs first
        wrapper.PlayEntryAnimation();
    }
    ...
    {
        if (transform.childCount != cards.Count || disableCardWrapper != lastDisableCardWrapper)
        {
            lastDisableCardWrapper = disableCardWrapper;
            InitCards();
        }
    ...
    private void SetCardsUILayers()
    {
        for (var i = 0; i < cards.Count; i++)
        {
            cards[i].uiLayer = cardConfig.defaultSortOrder + i;
        }
    }
    ...
    public void OnCardDragEnd()
    {
        ...
        {
            cardConfig?.OnCardPlayed?.Invoke(new CardPlayed(currentDraggedCard));
            if (cardConfig.destroyOnPlay)
            {
    ...
    {
        cards.Remove(card);
        cardConfig.OnCardDestroy?.Invoke(new CardDestroy(card));
        Destroy(card.gameObject);
    }

    private bool IsCursorInPlayArea()
    {
        if (cardConfig.playArea == null) return false;

        var cursorPosition = Input.mousePosition;
        var playArea = cardConfig.playArea;
