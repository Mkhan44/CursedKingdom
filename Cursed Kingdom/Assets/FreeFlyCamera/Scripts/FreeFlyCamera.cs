    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.PageUp))
        {
           ...
           _active = !_active;
            ...
            if (_active)
            { 
                Debug.Log("Freecam enabled. Click to disable cursor and press Esc to enable. Use Page Up to disable freecam."); 
            } 
            else 
            { 
                Debug.Log("Freecam disabled. Use Page Up to enable.");
            }
        }
        if (!_active)
            return;
